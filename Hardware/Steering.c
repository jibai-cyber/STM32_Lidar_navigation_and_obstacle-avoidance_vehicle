...
void TIM8_PWM_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_6|GPIO_Pin_7; //两个pwm通道
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化GPIOC_Pin_9
    ...
    TIM_OC2PreloadConfig(TIM8, TIM_OCPreload_Enable);						 //使能通道1输出
	
	TIM_Cmd(TIM8, ENABLE);						//使能计数器
	TIM_CtrlPWMOutputs(TIM8, ENABLE);	// 主输出使能，当使用的是通用定时器时，这句不需要
}

void Trun_SG90_angle(u8 angle)
{
    if(angle<=180) TIM_SetCompare1(TIM8,5+angle/9);
    else return;
}
