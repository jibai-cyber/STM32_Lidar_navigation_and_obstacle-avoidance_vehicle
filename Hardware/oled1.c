...
void OLED_I2C_Init(void)
{
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;  //这里注意引脚可能不同，需要再改.h文件里的内容
...
#define LONGEST_LEN 16  //设字体大小为16，则最多可有128/(16/2)==16==LONGEST_LEN个数字
void OLED_Show_Float_num(u8 x,u8 y,float num,u8 _f)    //_f：保留几位小数，设定最多显示16个数字,小数点后数字可能会存在失真问题，可多留几位小数。如0.01显示的是0.00999
{
    u8 _cnt,_num[LONGEST_LEN]="\0",cnt; //一定要数组初始赋值
    float _temp;
    int intg;
    ...
    if(_temp<=1 && _temp>=-1)
    {
        while(_temp<1 && _temp>-1)
        {
            _temp*=10;
            _cnt++;
            if(_cnt>_f)     //后续可以用作科学计数法
            {
    //            OLED_ShowNum(x,y,_cnt,2,16,1);
    //            OLED_ShowNum(x,y+16,intg,2,16,1);
                OLED_ShowString(x,y,"0.00",128*2/LONGEST_LEN);
                return;
            }
        }
        ...
        if(_temp>=0) intg = (int)_temp; //确保intg为非负整数
        else intg = (int)_temp*-1;
        
        for(u8 j=0;j<(_f-_cnt+1);j++)   //判断放在数组最后部分的数字（前面都是0或小数点，_cnt位后就是非0的），_f-_cnt就是剩余的需要扩张的数
        {
            _num[_f+1-j] = intg%10 + '0';
            intg/=10;
        }
        ...
        OLED_ShowString(x,y,_num,128*2/LONGEST_LEN);
    }
    else
    {
        for(u8 i=_f;i>0;i--)  _temp*=10;
        ...
        {
            cnt = _num[i];
            _num[i] = _num[_cnt-1-i];
            _num[_cnt-1-i] = cnt;
        }
        
        for(u8 i=0;i<_f;i++) _num[_cnt-i] = _num[_cnt-1-i];
    ...
    if(_temp<0) OLED_ShowChar(x-128/LONGEST_LEN,y,'-',128*2/LONGEST_LEN,1);   //一般要在前要预留一个负号位
    else OLED_ShowChar(x-128/LONGEST_LEN,y,' ',128*2/LONGEST_LEN,1);    //如果是非负要去掉负号
